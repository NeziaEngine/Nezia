...
左手系 Y-up を採用する（Unity 互換）。UE5 も左手系（Z-up）であり、左手系のメジャーエンジンとそのまま整合する。
...
- 内部演算はすべて `f32` 精度で行う
- リスナーの `right` は `normalize(cross(up, forward))` で導出される（左手系の cross 順序）
...
pub struct ListenerState {
    ...
    pub right:    [f32; 3],   // 派生値: normalize(cross(up, forward))（左手系）。SetListener 受信時に更新。
}
```
...
リスナーローカル空間に投影した方向ベクトルから水平成分の `sin(azimuth)` を取り出し、それをパン値としてイコールパワーパンニングを適用する。
...
2. リスナーローカル空間へ変換
   right   = normalize(cross(listener.up, listener.forward))   // 左手系
   forward = listener.forward
   local_x = dot(dir, right)    // 右方向成分
   local_z = dot(dir, forward)  // 前方向成分

3. 水平面投影でのパン値（= sin(azimuth)）
   horiz = sqrt(local_x^2 + local_z^2)
   pan   = (horiz > 0) ? clamp(local_x / horiz, -1, 1) : 0   // -1.0(左) 〜 1.0(右)
...
#### 設計判断: なぜ atan2 + clamp ではないか

直感的な実装として `azimuth = atan2(local_x, local_z)` をとり `clamp(azimuth, -π/2, π/2)` でパン角に落とす方法があるが、これは MVP では採用しない。問題点:

1. **後方半球で飽和**: `azimuth` の絶対値が `π/2` を超える領域（後方）では clamp により pan が常に ±1 に貼りつき、左後ろから右後ろまでの空間グラデーションが完全に消える。
2. **真後ろで不連続**: `atan2` は `local_z < 0` かつ `local_x` が符号反転する点（真後ろ）で `+π ↔ -π` に跳ぶ。clamp 後はその瞬間に pan が `+1 → -1` へ切り替わり、周回音源が真後ろを通過するときに「パチッ」と左右が反転して聞こえる。

`sin(azimuth)` 方式 (= `local_x / horiz`) は真後ろでも自動的に pan→0（センター）に向かうため両問題が解消する。代償として前後の定位は区別できなくなる（左前と左後で同じ pan になる）が、これはステレオ二点パンニングの原理的な限界であり、解消には HRTF（SP-13）が必要。
...
パンニング計算では毎ソースに `right = normalize(cross(up, forward))` を使うが、これはリスナーが変わらない限り定数値である。現在の設計のままではホットループ内で N ソース分 N 回計算してしまう。
