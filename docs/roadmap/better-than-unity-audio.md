...
| Compressor / Limiter | ○ | △ (Compressor 実装済 / master soft limiter 実装済、単体 Limiter エフェクト未) | 部分実装 |
...
| 3-5 | B | **DSP 拡充: ParamEQ / Compressor / Limiter** (Compressor は #21 で実装済、PeakingEq も実装済、master soft limiter も先行実装済。残るは単体 Limiter API) | 2 週間 |
