...
- **Source 単位** で LPF / HPF を挿入可能にする（Source 単位 Reverb はメモリ予算上 Phase 2 では拒否する。後述）。
...
- **Source 単位 Reverb** など重量級エフェクトの Source 挿入。論理的には統一 API で書けるが、Phase 2 ではコード上で拒否する（`MAX_REVERBS = 16` を超えるため）。
...
- Pre-Spatial と Post-Spatial で扱うチャネル数が異なる（モノラル / ステレオ）ため、エフェクト種別ごとに **どちら側で動作可能か** を実装側で属性として持つ。例: Reverb は Bus 専用、距離 LPF は Source Pre 専用、ParamEQ は両方可。
...
- Source 対象に Reverb を指定された場合、`add_effect` は `None` を返す（プール枯渇を待たず API 段階で拒否）。
...
impl SoundEngine {
    ...
    /// 拒否されるケース:
    /// - Source 対象 + Reverb 等の重量級エフェクト（`MAX_REVERB` プールが Bus 専用のため）
...
| Phase 3-3 Send / Sidechain | バス処理の `gain → effects → (ここに Send 挿入) → 親加算` の隙間を確保済み |
| Phase 3-5 ParamEQ / Compressor | `EffectKind` への variant 追加 + 専用 World 追加で完了 |
