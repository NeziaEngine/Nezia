...
バスはボイスの出力を集約し、音量・ミュートなどのパラメータをグループ単位で制御するためのミキシング経路である。
ボイスは必ずいずれかのバスに出力し、バスは別のバスへ出力することで木構造のルーティンググラフを形成する。
最終的にすべての信号はマスターバスに集約され、デバイスへ出力される。

```
Voice A ──→ SFX Bus ──→ Master Bus ──→ デバイス出力
Voice B ──→ SFX Bus ──┘       ↑
Voice C ──→ BGM Bus ──────────┘
Voice D ──→ BGM Bus ──┘
```
...
ボイスの `vol` と異なり、バスの `gain` は上限を `1.0` に制限しない。
バス単位での信号増幅（ブースト）はミキシングにおいて一般的な操作であり、制限する理由がない。
クリッピングの防止はマスターバス出力段のリミッター（将来実装）の責務とする。

## BusSystem

`VoicePoolSystem` と同じスパースセット方式で管理する System。
EntityId の発行・回収も BusSystem が自身で行う（VoicePoolSystem と同パターン）。

```
BusSystem
  ├── sparse / dense 管理（VoicePoolSystem と同構造）
...
フラット配列にする理由:
- `VoicePoolSystem::update()` に `&mut [f32]` を渡す際に、複数の可変参照が必要になる問題を回避できる。
...
## VoicePoolSystem との統合

### ボイスの出力先バス

`VoiceComponent` に `output_bus: u32` フィールドを追加する（密配列インデックス）。
...
2. VoicePoolSystem::update():
   各ボイスのサンプルを mix_buffer[voice.output_bus * stride ..] に加算
3. BusSystem::update():
   process_order 順に:
...
- ボイスの `vol` のみが個別音量として適用される。バスの `gain` はバス処理段で適用される。
...
- ゲームサウンドの主要なユースケース（BGM/SE/Voice/UI の分類制御）は木構造で十分カバーできる。
