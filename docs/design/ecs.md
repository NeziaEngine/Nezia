...
各 System が独自に EntityId を発行・管理する。Voice の EntityId と Bus の EntityId は
...
命名規則: **`〈名前〉System`**（例: `VoicePoolSystem`, `BusSystem`）
...
// 2. ボイスを各バスの mix_buffer に加算
{
    let mix_buf = bus_system.mix_buffer_mut();
    let stride = bus_system.bus_stride();
    voice_pool.update(mix_buf, stride, device_channels, device_sample_rate, &buffers);
}
...
impl VoicePoolSystem {
    /// BusSystem の mix_buffer に全アクティブボイスのサンプルを加算ミキシングし、
...
|---|---|---|
| Entity | — | `EntityId` |
| Component | `Component` | `VoiceComponent`, `BusComponent` |
| System | `System` | `VoicePoolSystem`, `BusSystem` |
| ファサード | — | `SoundEngine` |
| アセット | — | `AudioBuffer` |
